...
function buildSourceUrl(ref: string): string {
  const safeRef = ref.trim() || "main";
  return `https://raw.githubusercontent.com/clawdbot/nix-clawdbot/${safeRef}/nix/sources/clawdbot-source.nix`;
}
...
export async function fetchNixClawdbotSourceInfo(params: {
  ...
  const sourceUrl = buildSourceUrl(params.ref);
