...
const checks = [
  ...
  { scope: "lockdown", status: "missing", label: "infra state", detail: "missing" },
...
describe("doctor render", () => {
  it("renders report with grouped categories and summary", () => {
    ...
    expect(out).toMatch(/summary: ok=2 warn=1 missing=2/);
  ...
  it("renders gate failure summary", () => {
    const out = renderDoctorGateFailure({ checks, scope: "lockdown", strict: true });
    expect(out).toMatch(/doctor gate failed \(lockdown, strict\)/);
    expect(out).toMatch(/missing=2 warn=1/);
    expect(out).toMatch(/hint: run clawlets doctor --scope lockdown --strict/);
  });
});
