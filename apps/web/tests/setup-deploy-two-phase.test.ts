...
describe("setup deploy two-phase flow", () => {
  it("requires predeploy green before deploy and exposes both CTAs", () => {
    ...
    expect(source).toContain("bootstrapSecretsPayload.adminPassword = adminPassword")
    ...
    expect(source).toContain("Continue to verify")
    ...
    expect(source).toContain("setPredeployReadyFingerprint(predeployFingerprintRef.current)")
