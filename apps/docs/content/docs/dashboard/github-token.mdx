...
Clawlets uses <code>GITHUB_TOKEN</code> in Setup and during deploy/bootstrap whenever it
needs to access your project repository on GitHub.

If your GitHub repository is public, setup usually works without a token.
If your repository is private, token setup is required before Setup <code>Provider Tokens</code> can
complete.

Project creation in the dashboard creates the local project folder + local git repo only.
It does not create a GitHub repository. For remote/CI use, create a private GitHub repo,
add it as <code>origin</code>, and push your local project manually.

Use this page as the canonical operator flow for creating the token and saving it in the dashboard.

## Prereqs

- GitHub account with read access to the repository you use as your project remote (`origin`).
...
<Steps>
  ...
  <Step>
    Under <strong>Resource owner</strong>, select the owner that contains your project repo.
  ...
  <Step>
    ...
    Then under <strong>Selected repositories</strong>, select exactly the project repo Clawlets reads.
  </Step>
  <Step>
...
### `Repository appears private (404). Set GITHUB_TOKEN in your environment and retry.`

- Token is missing, expired, or lacks access to your project repo.
