...
1. `Server Access` (`connection`): requires `provisioning.adminCidr` and at least one `fleet.sshAuthorizedKeys` entry.
2. `Provider Tokens` (`creds`): requires `SOPS_AGE_KEY_FILE` and provider creds.
