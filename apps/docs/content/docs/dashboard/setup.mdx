...
<Steps>
  ...
  <Step>
    ...
    <code>GITHUB_TOKEN</code> gives the runner access to the project repository where
    Clawlets evaluates Nix. If that repo is private, the token is required.
    The evaluation source is:

    - <code>fleet/clawlets.json</code> → <code>baseFlake</code> when set (advanced override)
    - otherwise the repo <code>origin</code> remote of the local project folder
  </Step>
  <Step>
