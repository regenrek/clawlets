...
then starts one host-scoped setup operation. That operation seals the setup plan for the target
runner, executes `setup_apply`, and exposes structured step state before bootstrap.
...
<Steps>
  ...
  <Step>
    ...
    manual override for relay-only mode. In this step, save host-scoped
    <code>tailscale_auth_key</code>; Setup persists it to
    <code>secrets/hosts/&lt;host&gt;/tailscale_auth_key.yaml</code>.
  </Step>
  <Step>
    <strong>Install Server</strong>
    <br />
    Setup requires <code>GITHUB_TOKEN</code> to proceed. Predeploy starts one setup operation
    that runs staged config apply, deploy-creds write, bootstrap secrets init, and bootstrap secrets verify.
    After the setup operation is green, bootstrap runs.
  </Step>
  <Step>
...
### Setup apply operation failed

Open the host setup operation card and inspect the failed step. The UI shows safe step messages only,
not raw secret-bearing output. Fix the blocking input, then run <strong>Run predeploy</strong> again.
