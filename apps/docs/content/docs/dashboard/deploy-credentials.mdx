...
If you enable Tailscale lockdown (or set `hosts.<host>.tailnet.mode=tailscale`), the host
also needs `tailscale_auth_key` in `secrets/hosts/<host>/tailscale_auth_key.yaml`.
This key is managed in host Setup, not in project deploy credentials.
