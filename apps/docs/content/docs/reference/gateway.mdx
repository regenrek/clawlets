...
<Steps>
  <Step>Define gateway ids in <code>fleet.gatewayOrder</code> and <code>fleet.gateways</code>.</Step>
  <Step>Override a base port per gateway if needed via <code>fleet.gateways.&lt;id&gt;.profile.gatewayPort</code>.</Step>
...
- **Port collisions**: ensure `fleet.gatewayOrder` is stable and `gatewayPort` overrides do not overlap.
