...
- **Runtime (`<runtimeDir>/`)**: gitignored operator state + keys + provisioning artifacts.

Rules:
- `fleet/clawlets.json` is canonical; no secrets.
- Secrets live only in `secrets/` (sops-encrypted) and `<runtimeDir>/` (operator keys/runtime).
...
- Runtime: `<runtimeDir>/` (local operator state, gitignored)
