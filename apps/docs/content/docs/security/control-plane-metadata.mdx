...
- deploy credentials from `<runtimeDir>/env`
...
- Final setup start creates one host-scoped setup operation and enqueues one sealed `setup_apply` runner job for that operation.
...
- In setup/credentials UI, confirm provider tokens stay local (`<runtimeDir>/env`) and are shown as status only.
