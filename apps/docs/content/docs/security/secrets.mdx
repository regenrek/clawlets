...
- `tailscale_auth_key` (required when `hosts.<host>.tailnet.mode=tailscale`; set in
  host Setup `Tailscale lockdown`, stored at `secrets/hosts/<host>/tailscale_auth_key.yaml`)
