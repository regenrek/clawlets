...
- `<runtimeDir>/keys/operators/<operator>.agekey` (operator private key; local only)
...
- local operator age keypair in `<runtimeDir>/keys/operators/`
...
```bash
SOPS_AGE_KEY_FILE=<runtimeDir>/keys/operators/<you>.agekey \
