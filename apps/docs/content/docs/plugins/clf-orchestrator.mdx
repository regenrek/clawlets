...
- `@clawlets/clf-queue`: SQLite queue + schemas (idempotency, leases, retry).
- `clf-orchestrator`: daemon + worker loop + handlers.
- `clf`: gateway-facing CLI.
...
CLF is opt-in and shipped from the external `clf-cattle` repo.
Add the input and import its module explicitly in your project flake:

```nix
# flake.nix (inputs)
clf-cattle.url = "github:regenrek/clf-cattle";

# host/module imports
imports = [
  clawlets.nixosModules.clawletsProjectHost
  clf-cattle.nixosModules.clfOrchestrator
];
